...
async function chargerDonnees() {
  try {
    [livres, prets, demandes, utilisateurs] = await Promise.all([
      fetchData("http://localhost:3001/livres"),
      fetchData("http://localhost:3001/prets"),
      fetchData("http://localhost:3001/demandes"),
      fetchData("http://localhost:3001/utilisateurs")
    ]);
...
async function verifierCompte() {
  ...
  try {
    const prets = await fetchData("http://localhost:3001/prets");
...
async function supprimerPretsUtilisateur(nom) {
  ...
  for (const pret of pretsASupprimer) {
    await fetch(`http://localhost:3001/prets/${pret.id}`, {
      method: "DELETE",
    });
  }
}
...
async function modifier() {
  ...
  await fetch(`http://localhost:3001/utilisateurs/${user.id}`, {
    method: "PATCH",
  ...
  await fetch("http://localhost:3001/changement-mot-de-passe", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function afficherFavoris() {
  ...
  for (const favori of favorisUtilisateur) {
    try {
      const response = await fetch(`http://localhost:3001/livres/${favori.livreId}`);
...
async function retourLivre(pretId) {
  ...
  try {
    const response = await fetch(`http://localhost:3001/prets/${pretId}`, {
      method: "PATCH",
      headers: {"Content-Type": "application/json"},
    ...
    if (pret) {
      ...
      if (livre) {
        ...
        await fetch(`http://localhost:3001/livres/${livre.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
...
async function annulerDemande(demandeId) {
  ...
  try {
    const response = await fetch(`http://localhost:3001/demandes/${demandeId}`, {
      method: "DELETE",
      headers: {"Content-Type": "application/json"}
    });
    ...
    setTimeout(async () => {
      demandes = await fetchData("http://localhost:3001/demandes");
      afficherDemandes(utilisateur);
