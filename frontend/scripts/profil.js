...
async function chargerDonnees() {
  try {
    [livres, prets, demandes, utilisateurs] = await Promise.all([
      fetchData("https://bibooks-backend.vercel.app/livres"),
      fetchData("https://bibooks-backend.vercel.app/prets"),
      fetchData("https://bibooks-backend.vercel.app/demandes"),
      fetchData("https://bibooks-backend.vercel.app/utilisateurs")
    ]);
...
async function verifierCompte() {
  ...
  try {
    const prets = await fetchData("https://bibooks-backend.vercel.app/prets");
...
async function supprimerPretsUtilisateur(nom) {
  ...
  for (const pret of pretsASupprimer) {
    await fetch(`https://bibooks-backend.vercel.app/prets/${pret.id}`, {
      method: "DELETE",
    });
  }
}
...
async function modifier() {
  ...
  await fetch(`https://bibooks-backend.vercel.app/utilisateurs/${user.id}`, {
    method: "PATCH",
  ...
  await fetch("https://bibooks-backend.vercel.app/changement-mot-de-passe", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
async function afficherFavoris() {
  ...
  for (const favori of favorisUtilisateur) {
    try {
      const response = await fetch(`https://bibooks-backend.vercel.app/livres/${favori.livreId}`);
...
async function retourLivre(pretId) {
  ...
  try {
    const response = await fetch(`https://bibooks-backend.vercel.app/prets/${pretId}`, {
      method: "PATCH",
      headers: {"Content-Type": "application/json"},
    ...
    if (pret) {
      ...
      if (livre) {
        ...
        await fetch(`https://bibooks-backend.vercel.app/livres/${livre.id}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
...
async function annulerDemande(demandeId) {
  ...
  try {
    const response = await fetch(`https://bibooks-backend.vercel.app/demandes/${demandeId}`, {
      method: "DELETE",
      headers: {"Content-Type": "application/json"}
    });
    ...
    setTimeout(async () => {
      demandes = await fetchData("https://bibooks-backend.vercel.app/demandes");
      afficherDemandes(utilisateur);
